  ...
  function getLastBlocksbyTime() {
    ...
    $stmt = $this->mysqli->prepare("
      SELECT
        ...
        IFNULL(SUM(IF(confirmations > -1 AND price IS NOT NULL AND FROM_UNIXTIME(time) >= DATE_SUB(now(), INTERVAL 3600 SECOND), amount * price, 0)), 0) AS 1HourBTC,
        ...
        IFNULL(SUM(IF(confirmations > -1 AND price IS NOT NULL AND FROM_UNIXTIME(time) >= DATE_SUB(now(), INTERVAL 86400 SECOND), amount * price, 0)), 0) AS 24HourBTC,
        ...
        IFNULL(SUM(IF(confirmations > -1 AND price IS NOT NULL AND FROM_UNIXTIME(time) >= DATE_SUB(now(), INTERVAL 604800 SECOND), amount * price, 0)), 0) AS 7DaysBTC,
        ...
        IFNULL(SUM(IF(confirmations > -1 AND price IS NOT NULL AND FROM_UNIXTIME(time) >= DATE_SUB(now(), INTERVAL 2419200 SECOND), amount * price, 0)), 0) AS 4WeeksBTC,
